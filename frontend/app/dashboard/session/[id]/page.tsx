...
const PollCard = memo(function PollCard({ poll, index, stats, onUpdate, onOpenDetail }: PollCardProps) {
  const total = stats.correct + stats.wrong

  // Jangan mutasi props/state dengan .sort() langsung
  const sortedOptions = useMemo(
    () => (poll.options ? [...poll.options].sort((a, b) => a.option_order - b.option_order) : []),
    [poll.options],
  )
...
function SessionPage() {
  ...
  useEffect(() => {
    ...
    const startFallback = () => {
      if (!fallbackTimer) fallbackTimer = setInterval(() => scheduleFlush(), FALLBACK_POLL_MS)
    }
    const stopFallback = () => {
    ...
    const loadInitial = async () => {
      ...
      if (pollRes.status === "fulfilled") {
        ...
        setPolls(loaded)
        loaded.filter((p) => p.type === "quiz").forEach((p) => quizIds.add(p.id))
  ...
  const onUpdateSingle = useCallback(
    async (pollId: string, statusTarget: Poll["status"]) => {
      ...
      try {
        const updated = await updateSinglePolls(pollId, statusTarget === "closed" ? "closed" : "published", null)
        setPolls((prev) =>
          prev ? prev.map((p) => (p.id === updated.id ? { ...updated, options: updated.options || p.options } : p)) : null,
        )
  ...
  const onOpenDetail = useCallback(
    (poll: Poll) => {
      router.push(`/dashboard/session/${sessionId}/poll/${poll.id}/${poll.type === "qa" ? "qa" : "wordcloud"}`)
    },
    [router, sessionId],
  )

  const toggleSessionStatus = async (statusTarget: SessionData["status"]) => {
    if (!isAuthenticated) return setErrorMessage("Sesi login tidak sesuai atau sudah kedaluwarsa.")
    try {
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="bg-white border rounded-xl p-5">
          <div className="flex justify-between items-start">
            <div>
              <p className="text-sm text-gray-500 mt-1">
                ...
                <span className="font-mono font-semibold bg-gray-100 px-2 py-0.5 rounded">{session?.access_code}</span>
              </p>
            </div>
            <span
              className={`text-xs font-medium px-2.5 py-1 rounded-full ${session?.status === "active" ? "bg-green-100 text-green-700" : "bg-gray-200 text-gray-600"
                }`}
            >
          ...
          <div className="flex gap-2 mt-4">
            ...
            <button
              onClick={() => router.push(`/dashboard/createpolls/${session?.id}`)}
        ...
        {successMessage && (
          <div className="bg-green-50 border border-green-200 text-green-700 text-sm p-3 rounded-lg">{successMessage}</div>
        )}
